...
- **Strapi v5 Blocks field type and markdown converters** ([#51](https://github.com/MehdiZare/strapi-kit/issues/51))
  ...
  - Lossy cases (underline, missing image/link URL, unknown or malformed nodes) are recorded in `lossy_reasons` (deduplicated; empty iff faithful). Markdown metacharacters in text leaves are escaped before marks are applied
